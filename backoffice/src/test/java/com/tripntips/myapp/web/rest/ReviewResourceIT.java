...
import static com.tripntips.myapp.web.rest.TestUtil.sameInstant;
...
class ReviewResourceIT {
    ...
    public static Review createEntity(EntityManager em) {
        Review review = new Review().rating(DEFAULT_RATING).content(DEFAULT_CONTENT).createdAt(DEFAULT_CREATED_AT);
        // Add required entity
        ...
        if (TestUtil.findAll(em, Recommendation.class).isEmpty()) {
            recommendation = RecommendationResourceIT.createEntity(em);
            em.persist(recommendation);
            em.flush();
        } else {
            recommendation = TestUtil.findAll(em, Recommendation.class).get(0);
        }
    ...
    public static Review createUpdatedEntity(EntityManager em) {
        Review review = new Review().rating(UPDATED_RATING).content(UPDATED_CONTENT).createdAt(UPDATED_CREATED_AT);
        // Add required entity
        User user = UserResourceIT.createEntity(em);
        em.persist(user);
        em.flush();
        ...
        Recommendation recommendation;
        if (TestUtil.findAll(em, Recommendation.class).isEmpty()) {
            recommendation = RecommendationResourceIT.createUpdatedEntity(em);
            em.persist(recommendation);
            em.flush();
        } else {
            recommendation = TestUtil.findAll(em, Recommendation.class).get(0);
    ...
    void putNewReview() throws Exception {
        ...
        updatedReview.rating(UPDATED_RATING).content(UPDATED_CONTENT).createdAt(UPDATED_CREATED_AT);
    ...
    void partialUpdateReviewWithPatch() throws Exception {
        ...
        partialUpdatedReview.rating(UPDATED_RATING).content(UPDATED_CONTENT);
    ...
    void fullUpdateReviewWithPatch() throws Exception {
        ...
        partialUpdatedReview.rating(UPDATED_RATING).content(UPDATED_CONTENT).createdAt(UPDATED_CREATED_AT);
