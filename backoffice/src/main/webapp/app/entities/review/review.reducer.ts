...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_REVIEW: 'review/PARTIAL_UPDATE_REVIEW',
...
export default (state: ReviewState = initialState, action): ReviewState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_REVIEW):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_REVIEW):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_REVIEW):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<IReview> = (page, size, sort) => ({
  ...
  payload: axios.get<IReview>(`${apiUrl}?cacheBuster=${new Date().getTime()}`),
});
...
export const partialUpdate: ICrudPutAction<IReview> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_REVIEW,
