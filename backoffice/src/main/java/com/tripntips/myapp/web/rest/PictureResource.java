...
import com.tripntips.myapp.repository.PictureRepository;
...
/**
 * REST controller for managing {@link com.tripntips.myapp.domain.Picture}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PictureResource {
    ...
    public ResponseEntity<Picture> updatePicture(@PathVariable(value = "id", required = false) final Long id, @RequestBody Picture picture)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, picture.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Picture> partialUpdatePicture(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, picture.getId().toString())
        );
    }
